...
class FirebaseChatCore {
  ...
  Future<types.Room?> getRoomByDependId(String dependId) async {
    final fu = firebaseUser;
    ...
    if (query.docs.isNotEmpty) {
      // Lấy DocumentSnapshot của tài liệu đầu tiên
  ...
  Future<types.Room?> getRoomAdmin() async {
    final fu = firebaseUser;
  ...
  Future<types.User?> getUser(num partnerId) async {
    ...
    final query = await getFirebaseFirestore()
        .collection(config.usersCollectionName)
        .where('metadata.partnerId', isEqualTo: partnerId)
        .limit(1)
        .get();
  ...
  Future<types.User?> getUserAdmin() async {
    final fu = firebaseUser;
    ...
    final query = await getFirebaseFirestore()
        .collection(config.usersCollectionName)
        .where('role', isEqualTo: types.Role.agent.toShortString())
        .limit(1)
        .get();
